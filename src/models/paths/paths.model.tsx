...
export interface PathNode {
  ...
  x: number;
  y: number;
  // Optionally, anchor points for curves (for Bézier, arc, etc.)
  controlPoints?: { x: number; y: number }[];
  // For intersection/connection logic
  type?: 'end' | 'connect' | 'intersection' | 'main';
}

// Supported geometry for a segment
export type PathSegmentGeometry =
  | { type: 'line' } // straight line
  | { type: 'bezier'; controlPoints: { x: number; y: number }[] } // C/S/Q/T
  | { type: 'arc'; rx: number; ry: number; xAxisRotation: number; largeArcFlag: boolean; sweepFlag: boolean };

// Usage flags for a path/segment
...
export interface PathSegment {
  ...
  usage: PathUsage;
  speedLimit?: number; // km/h or mph
  // Optionally, override inherited properties
  [key: string]: unknown;
}

// A path is a sequence of segments (e.g. a lane, sidewalk, etc.)
export interface Path {
  id: string;
  nodeIds: string[]; // ordered
  segmentIds: string[]; // ordered
  usage: PathUsage;
  width: number; // meters
  material: PathMaterial;
  speedLimit?: number;
...
// A road is a bundle of paths (sidewalk, lanes, bike lane, etc.)
export interface Road {
  id: string;
  name?: string;
  pathIds: string[]; // ordered left-to-right
  segmentIds: string[]; // all segments in this road
  // Shared properties
  speedLimit?: number;
  material?: PathMaterial;
  // Optionally, override inherited properties
...
export type IntersectionType = 'none' | 'stop' | 'yield' | 'light';

export interface Intersection {
  nodeId: string;
  type: IntersectionType;
  // Optionally, more info (e.g. which directions have stop, etc.)
  [key: string]: unknown;
}

// --- End Road/Path Model Types ---